...
class STGCNGraphConv(nn.Module):
    ...
    def __init__(self, Kt, Ks, blocks, T, n_vertex, act_func, graph_conv_type, filter, bias, droprate):
        ...
        for l in range(len(blocks) - 3):
            modules.append(STConvBlock(Kt, Ks, n_vertex, blocks[l][-1], blocks[l+1], act_func, graph_conv_type, filter, bias, droprate))
...
def get_origin_stgcn(args, gso):
    ...
    blocks.append([1])
    model = STGCNGraphConv(Kt, Ks, blocks, T, n_vertex, act_func, graph_conv_type, gso, bias, droprate)
    return model